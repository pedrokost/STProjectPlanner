...
class ProjectPlannerTrelloUp(sublime_plugin.TextCommand):
    ...
    def show_token_expired_help(self, e):
        print("It seems your token is invalid or has expired, try adding it again.\nToken URL: %s" %
              self.token_url(), "The error encountered was: '%s'" % e)
...
class ProjectPlannerTrello(sublime_plugin.TextCommand):
    ...
    def insert_missing_cards(self, cards, section, edit):
        ...
        if index == -1:
            print('WARNING: for some reason couldn\'t find location to insert the section {}'.format(
                section, section.all_tasks[-1]))
    ...
    def add_missing_cards(self, connection, edit, matches):
        ...
        for pair in matches:
            ...
            missing_cards = [card for card in pair.list.cards if not has_match(
                self.url_core(card.url), pair.section.lines)]

            incorrect_list_tasks = [task for task in pair.section.all_tasks if task.is_trello_card and not has_match(
                self.url_core(task.trello_url), card_urls)]
    ...
    def __compute_checkitem_duration(self, item):
        ...
        for category, dict in meta.categories.items():
            ...
            if meta.categories[category]['duration_value'][0] is not None and meta.categories[category]['duration_unit'] is not None:
                resp[category] = meta.categories[category]['duration_value'][0] * \
                    Section.DURATION_MAP[meta.categories[category]['duration_unit']]
    ...
    def __update_card_metadata(self, connection, edit, task, section_title):
        ...
        for dur in card_durations:
            ...
            card_duration_human += '{} {} '.format(category, human_duration(dur.value,
                                                                            Section.DURATION_MAP, max_segments=1))
        ...
        for index_idx, index in enumerate(indices):
            if index_idx > 0:
                if indices[index_idx - 1] > index:
                    self.add_error(
                        'List ordering', '*{}* should be placed before *{}*'.format(list_titles[index_idx - 1], list_titles[index_idx]))
